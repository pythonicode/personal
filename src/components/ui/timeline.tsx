    ...
    <div className="w-full bg-background font-sans md:px-10" ref={containerRef}>
      <div ref={ref} className="relative max-w-7xl mx-auto pb-20">
        {data.toReversed().map((item, index) => (
          <div key={index} className="flex justify-start py-8 md:gap-10">
            <div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-full">
              ...
              <h3 className="hidden md:block text-xl md:pl-20 md:text-5xl font-bold text-foreground ">
                {item.title}
                <p className="text-muted-foreground text-sm font-normal mt-2">{item.description}</p>
              </h3>
            </div>
        ...
        >
          <motion.div
            ...
            className="absolute inset-x-0 top-0  w-[2px] bg-gradient-to-t from-foreground via-muted-foreground to-transparent from-[0%] via-[10%] rounded-full"
          />
        </div>
      </div>
    </div>
