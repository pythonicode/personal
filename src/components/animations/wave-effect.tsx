...
export function WaveEffect({ children, color, className, duration = 0.5 }: WaveEffectProps) {
    ...
    >
      ...
      {showRipple && (
        <motion.span
          key={keyValue}
          className={cn(
            'absolute aspect-square bg-black dark:bg-white bg-opacity-5 dark:bg-opacity-10 rounded-full pointer-events-none start-0 top-0',
            color,
          )}
          initial={{
            opacity: direction === 'in' ? 1 : 0,
            scale: direction === 'in' ? 2.5 : 0.5,
          }}
          animate={{
            opacity: direction === 'in' ? 0 : 1,
            scale: direction === 'in' ? 0.5 : 2.5,
          }}
